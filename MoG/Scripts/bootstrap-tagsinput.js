...
(function ($) {
  ...
  TagsInput.prototype = {
    ...
    add: function(item, dontPushVal) {
      ...
      // add a tag element
      var isRemovable = (itemText.indexOf('#') == itemText.length-1);
      var $htmlTag = '<span class="tag ' + htmlEncode(tagClass) + '">' + htmlEncode(itemText);
      if (isRemovable)
      {
         $htmlTag = $htmlTag.substr(0,$htmlTag.length-1);
         $htmlTag += '<span data-role="remove"></span></span>';
      }
      else
         $htmlTag += '<span data-role=""></span></span>';
      var $tag = $($htmlTag);
      $tag.data('item', item);
